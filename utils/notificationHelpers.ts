import type {
  Notification,
  NotificationEnrichedData,
} from "@/types/notifications";

/**
 * Parse enriched data from notification payload and metadata
 */
export function parseNotificationEnrichedData(
  notification: Notification,
): NotificationEnrichedData | null {
  try {
    ...
    if (notification.payload) {
      ...
      if (typeof notification.payload === "string") {
        try {
          ...
          console.error(
            "[parseNotificationEnrichedData] Failed to parse payload string:",
            e,
          );
        }
    ...
    if (notification.metadata) {
      ...
      if (typeof notification.metadata === "string") {
        try {
          ...
          console.error(
            "[parseNotificationEnrichedData] Failed to parse metadata string:",
            e,
          );
        }
...
export function hasEnrichedData(notification: Notification): boolean {
  return (
    !!notification.enriched && Object.keys(notification.enriched).length > 0
  );
}
